...
def ws_connect(message):
    # reply_channel is another type of channel that represents the users channel,
    # It informs to which channel the response should be send back.
    # Every client has its own reply_channel
    Group('stocks').add(message.reply_channel)
    #Group('stocks').send({'text':'connected'})

    json_data = json.dumps({'user': 'Sush', 'email': 'sush@gmail.com'})

    Group('stocks').send({'text': json_data})
